...
# rocket is the model's own -- the BTR_Missile parts on the rails, copied into a
# node of their own at launch and hidden on the rails until the reload is done.
#
...
var _centre := Vector3.ZERO     # the missile's middle, pivot space
...
func _ready() -> void:
	_rng.seed = 2
	_base = btr.launcher_node()
	_base_rest = _base.transform
	_pivot = _base.find_child("BTR_LauncherPivot", true, false)
	var nose: Node3D = null
	var tail: Node3D = null
	for child in _pivot.get_children():
		if child is MeshInstance3D and String(child.name).begins_with("BTR_Missile"):
			_parts.append(child)
			if child.name == "BTR_MissileNose":
				nose = child
			elif child.name == "BTR_MissileTail":
				tail = child
	# The parts are modelled along the rails; which way is forward is read off
	# where the nose and the tail are, not assumed.
	var body := _pivot.get_node("BTR_Missile") as MeshInstance3D
	_centre = body.position
	_axis = (nose.position - tail.position).normalized()
	var extent := 0.0
	for part in _parts:
		var box: AABB = part.transform * part.get_aabb()
		for i in 8:
			extent = maxf(extent, (box.get_endpoint(i) - _centre).dot(_axis))
	_nose = extent
	_tail = (tail.position - _centre).dot(_axis)
...
func _traverse(delta: float) -> void:
	var budget := TRAVERSE_RATE * delta
...
func _launch() -> void:
	...
	# The model's parts, re-hung on a node of their own about the missile's
	# middle, then turned from the rails' line on to the flight's.
	...
	_stretch_flame(flame, 0.18)

	_rockets.append({"node": rocket, "flame": flame, "direction": direction,
			"speed": LAUNCH_SPEED, "left": (target - start).length(), "smoke": 0.0,
			"scale": frame.basis.get_scale().x})
...
func _fly(rocket: Dictionary, delta: float) -> void:
	...
	_stretch_flame(flame, _rng.randf_range(0.16, 0.3))
	rocket.smoke += delta
	while rocket.smoke >= SMOKE_EVERY:
		rocket.smoke -= SMOKE_EVERY
		_trail(node.global_position + direction * _tail * rocket.scale)
...
func _stretch_flame(flame: MeshInstance3D, length: float) -> void:
	var along := Basis(Quaternion(Vector3.RIGHT, _axis)) if Vector3.RIGHT.cross(_axis).length() > 1e-5 \
			else Basis.from_euler(Vector3(0, PI if _axis.x < 0.0 else 0.0, 0))
